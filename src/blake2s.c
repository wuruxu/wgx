 ...
 * BLAKE2s - based on the official reference implementation (CC0 / public domain)
...
static int blake2s_init_param(blake2s_state *S, const blake2s_param *P) {
    ...
    for (int i = 0; i < 8; i++)
        S->h[i] = blake2s_IV[i] ^ load32(p + sizeof(S->h[i]) * i);
    return 0;
}
...
int blake2s_init_key(blake2s_state *S, size_t outlen,
    ...
    /* prepend key as first block */
    uint8_t block[BLAKE2S_BLOCKBYTES];
    memset(block, 0, sizeof(block));
    memcpy(block, key, keylen);
    return blake2s_update(S, block, BLAKE2S_BLOCKBYTES);
}
...
int blake2s_final(blake2s_state *S, void *out, size_t outlen) {
    ...
    /* set last block flag */
    S->f[0] = (uint32_t)-1;
    if (S->last_node) S->f[1] = (uint32_t)-1;
...
int blake2s(void *out, size_t outlen,
    ...
    if (ret < 0) return ret;
    if (blake2s_update(&S, in, inlen) < 0) return -1;
