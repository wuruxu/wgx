...
#define wg_iphdr ip
#define wg_ip_src ip_src
#define wg_ip_dst ip_dst
#else
#define wg_iphdr iphdr
#define wg_ip_src saddr
#define wg_ip_dst daddr
#endif

#ifdef WGX_APPLE_BACKEND
...
void wg_log(wg_device_t *dev, int level, const char *fmt, ...) {
    ...
    va_end(ap);
    wgx_apple_log(level == LOG_ERROR ? 1 : 0, msg);
#else
    struct timespec ts;
...
static wg_rx_buffer_t *rx_buffer_from_data(char *base) {
    return (wg_rx_buffer_t *)(base - offsetof(wg_rx_buffer_t, data));
}

static wg_rx_buffer_t *rx_buffer_acquire(wg_device_t *dev) {
    pthread_mutex_lock(&dev->rx_buffer_pool_lock);
    wg_rx_buffer_t *buf = dev->rx_buffer_pool;
    if (buf)
        dev->rx_buffer_pool = buf->next;
    pthread_mutex_unlock(&dev->rx_buffer_pool_lock);
...
static void rx_buffer_release(wg_device_t *dev, wg_rx_buffer_t *buf) {
    if (!buf)
        return;
    if (!buf->pooled) {
        free(buf);
    ...
    pthread_mutex_lock(&dev->rx_buffer_pool_lock);
    buf->next = dev->rx_buffer_pool;
    dev->rx_buffer_pool = buf;
    pthread_mutex_unlock(&dev->rx_buffer_pool_lock);
}
...
static void handle_transport(wg_device_t *dev,
    ...
    wg_rx_buffer_t *ptbuf = rx_buffer_acquire(dev);
    if (!ptbuf) return;
    uint8_t *plaintext = ptbuf->data;
    ...
    if (ptlen > 0) {
        ...
        if (dev->socks5_mode && dev->tcp_worker) {
            ...
            if (ptlen >= 1) {
                uint8_t version = (plaintext[0] >> 4) & 0xf;
                if (version == 4 && ptlen >= sizeof(struct wg_iphdr)) {
                    struct wg_iphdr *ip4 = (struct wg_iphdr *)plaintext;
                    struct in_addr dst;
                    memcpy(&dst, &ip4->wg_ip_src, sizeof(dst));
...
static void on_udp_recv(uv_udp_t *handle,
    ...
    if (nread < 0) {
        ...
        udp_recv_buffer_release(dev, buf);
        return;
    }
    if (nread == 0) {
        udp_recv_buffer_release(dev, buf);
    ...
    if (!addr) { udp_recv_buffer_release(dev, buf); return; }
...
static void on_tun_readable(uv_poll_t *handle, int status, int events) {
    ...
    while ((n = tun_read(dev->tun_fd, buf, sizeof(buf))) > 0) {
        ...
        if (pktlen >= 1) {
            ...
            if (version == 4 && pktlen >= sizeof(struct wg_iphdr)) {
                struct wg_iphdr *ip4 = (struct wg_iphdr *)buf;
                struct in_addr dst;
                memcpy(&dst, &ip4->wg_ip_dst, sizeof(dst));
                peer = allowedips_lookup_v4(&dev->allowedips, &dst);
...
int device_init(wg_device_t *dev, const char *ifname, uv_loop_t *loop) {
    ...
    pthread_mutex_init(&dev->rx_buffer_pool_lock, NULL);
    ...
    dev->rx_buffer_nodes = calloc(WG_RX_BUFFER_POOL_SIZE, sizeof(*dev->rx_buffer_nodes));
    if (!dev->rx_buffer_nodes)
        return -1;
    for (size_t i = 0; i < WG_RX_BUFFER_POOL_SIZE; i++) {
        dev->rx_buffer_nodes[i].pooled = 1;
        dev->rx_buffer_nodes[i].next = dev->rx_buffer_pool;
        dev->rx_buffer_pool = &dev->rx_buffer_nodes[i];
    }
