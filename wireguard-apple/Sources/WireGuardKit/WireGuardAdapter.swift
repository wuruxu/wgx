...
public class WireGuardAdapter {
    ...
    private static func diagnosticSummary(for tunnelConfiguration: TunnelConfiguration) -> String {
        let interface = tunnelConfiguration.interface
        let listenPort = interface.listenPort.map { String($0) } ?? "<auto>"
        let peerEndpointCount = tunnelConfiguration.peers.filter { $0.endpoint != nil }.count
        let allowedIPCount = tunnelConfiguration.peers.reduce(0) { $0 + $1.allowedIPs.count }
        return "name=\(tunnelConfiguration.name ?? "<nil>"), addresses=\(interface.addresses.count), dns=\(interface.dns.count), dnsSearch=\(interface.dnsSearch.count), listenPort=\(listenPort), mtu=\(interface.mtu.map { String($0) } ?? "<auto>"), peers=\(tunnelConfiguration.peers.count), endpoints=\(peerEndpointCount), allowedIPs=\(allowedIPCount)"
    }

    private static func diagnosticSummary(forUapiConfig wgConfig: String) -> String {
        let lines = wgConfig.split(separator: "\n")
        let peerCount = lines.filter { $0.hasPrefix("public_key=") }.count
        let endpointCount = lines.filter { $0.hasPrefix("endpoint=") }.count
        let allowedIPCount = lines.filter { $0.hasPrefix("allowed_ip=") }.count
        let listenPort = lines.first { $0.hasPrefix("listen_port=") }?.split(separator: "=", maxSplits: 1).last.map(String.init) ?? "<none>"
        let replacePeers = lines.first { $0.hasPrefix("replace_peers=") }.map(String.init) ?? "<none>"
        return "lines=\(lines.count), listenPort=\(listenPort), peers=\(peerCount), endpoints=\(endpointCount), allowedIPs=\(allowedIPCount), \(replacePeers)"
    }

    private static func diagnosticDescription(for error: WireGuardAdapterError) -> String {
        switch error {
        case .cannotLocateTunnelFileDescriptor:
        ...
        case .dnsResolution(let errors):
            return "dnsResolution(\(errors.map { "\($0.address): \($0.errorDescription ?? "<nil>")" }.joined(separator: ", ")))"
        case .setNetworkSettings(let systemError):
            return "setNetworkSettings(\(systemError.localizedDescription))"
    ...
    public func start(tunnelConfiguration: TunnelConfiguration, completionHandler: @escaping (WireGuardAdapterError?) -> Void) {
        workQueue.async {
            guard case .stopped = self.state else {
                self.logHandler(.error, "WireGuardAdapter.start rejected because adapter state is not stopped")
                completionHandler(.invalidState)
                return
            }

            self.logHandler(.verbose, "WireGuardAdapter.start configuration summary: \(Self.diagnosticSummary(for: tunnelConfiguration))")
            ...
            do {
                self.logHandler(.verbose, "WireGuardAdapter.start resolving endpoints")
                let settingsGenerator = try self.makeSettingsGenerator(with: tunnelConfiguration)
                self.logHandler(.verbose, "WireGuardAdapter.start applying network settings")
                try self.setNetworkSettings(settingsGenerator.generateNetworkSettings())

                let (wgConfig, resolutionResults) = settingsGenerator.uapiConfiguration()
                self.logEndpointResolutionResults(resolutionResults)

                self.logHandler(.verbose, "WireGuardAdapter.start starting backend with generated UAPI config summary: \(Self.diagnosticSummary(forUapiConfig: wgConfig))")
                self.state = .started(
                    try self.startWireGuardBackend(wgConfig: wgConfig),
                    settingsGenerator
                )
                self.networkMonitor = networkMonitor
                self.logHandler(.verbose, "WireGuardAdapter.start completed successfully")
                completionHandler(nil)
            } catch let error as WireGuardAdapterError {
                self.logHandler(.error, "WireGuardAdapter.start failed: \(Self.diagnosticDescription(for: error))")
                networkMonitor.cancel()
    ...
    private func startWireGuardBackend(wgConfig: String) throws -> Int32 {
        ...
        self.logHandler(.verbose, "WireGuardAdapter.startWireGuardBackend using tunnel fd \(tunnelFileDescriptor)")
        let handle = wgTurnOn(wgConfig, tunnelFileDescriptor)
        if handle < 0 {
            self.logHandler(.error, "WireGuardAdapter.startWireGuardBackend wgTurnOn failed with code \(handle)")
            throw WireGuardAdapterError.startWireGuardBackend(handle)
        }
        self.logHandler(.verbose, "WireGuardAdapter.startWireGuardBackend wgTurnOn returned handle \(handle)")
        #if os(iOS)
        wgDisableSomeRoamingForBrokenMobileSemantics(handle)
        #endif
        return handle
