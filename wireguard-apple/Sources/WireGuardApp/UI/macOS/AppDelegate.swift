...
final class SystemExtensionInstaller: NSObject, OSSystemExtensionRequestDelegate {
    ...
    private var extensionIdentifier: String {
        let appIdentifier = Bundle.main.bundleIdentifier ?? "com.github.wuruxu.wgx"
        return "\(appIdentifier).network-extension"
    }
    private var hasSubmittedActivationRequest = false
    private var hasShownUserApprovalAlert = false

    func activate() {
        guard !hasSubmittedActivationRequest else { return }
        hasSubmittedActivationRequest = true

        let request = OSSystemExtensionRequest.activationRequest(forExtensionWithIdentifier: extensionIdentifier, queue: .main)
        request.delegate = self
        wg_log(.info, message: "Submitting system extension activation request for \(extensionIdentifier)")
        OSSystemExtensionManager.shared.submitRequest(request)
    }

    func request(_ request: OSSystemExtensionRequest, didFinishWithResult result: OSSystemExtensionRequest.Result) {
        wg_log(.info, message: "System extension activation request finished with result \(String(describing: result))")
    }

    func request(_ request: OSSystemExtensionRequest, didFailWithError error: Error) {
        hasSubmittedActivationRequest = false
        wg_log(.error, message: "System extension activation request failed: \(error.localizedDescription)")
    }

    func requestNeedsUserApproval(_ request: OSSystemExtensionRequest) {
        wg_log(.info, staticMessage: "System extension activation request needs user approval")
        showUserApprovalAlertIfNeeded()
    }

    func request(_ request: OSSystemExtensionRequest, actionForReplacingExtension existing: OSSystemExtensionProperties, withExtension ext: OSSystemExtensionProperties) -> OSSystemExtensionRequest.ReplacementAction {
        wg_log(.info, message: "Replacing system extension \(existing.bundleShortVersion) with \(ext.bundleShortVersion)")
        return .replace
    }

    private func showUserApprovalAlertIfNeeded() {
        guard !hasShownUserApprovalAlert else { return }
        hasShownUserApprovalAlert = true
        ...
        DispatchQueue.main.async {
            ...
            let alert = NSAlert()
            alert.messageText = tr("macSystemExtensionApprovalRequiredTitle")
            alert.informativeText = tr("macSystemExtensionApprovalRequiredMessage")
