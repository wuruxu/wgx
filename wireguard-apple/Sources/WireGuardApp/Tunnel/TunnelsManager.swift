...
private func lastErrorTextFromNetworkExtension(for tunnel: TunnelContainer) -> (title: String, message: String)? {
    guard let lastErrorFileURL = FileManager.networkExtensionLastErrorFileURL else {
        wg_log(.error, message: "Activation failed for tunnel '\(tunnel.name)' but app group last-error URL is unavailable")
        return nil
    }
    guard let lastErrorData = try? Data(contentsOf: lastErrorFileURL) else {
        wg_log(.debug, message: "Activation failed for tunnel '\(tunnel.name)' but no network extension last-error file exists at \(lastErrorFileURL.path)")
        return nil
    }
    guard let lastErrorText = String(data: lastErrorData, encoding: .utf8) else {
        wg_log(.error, message: "Activation failed for tunnel '\(tunnel.name)' but last-error file is not UTF-8 at \(lastErrorFileURL.path)")
        return nil
    }
    let lastErrorStrings = lastErrorText.splitToArray(separator: "\n")
    guard lastErrorStrings.count == 2 else {
        wg_log(.error, message: "Activation failed for tunnel '\(tunnel.name)' but last-error file has unexpected format: \(lastErrorText)")
        return nil
    }
    guard tunnel.activationAttemptId == lastErrorStrings[0] else {
        wg_log(.debug, message: "Ignoring stale network extension last-error for tunnel '\(tunnel.name)': expected activationAttemptId=\(tunnel.activationAttemptId ?? "<nil>"), file activationAttemptId=\(lastErrorStrings[0]), error=\(lastErrorStrings[1])")
        return nil
    }

    wg_log(.error, message: "Network extension reported activation failure for tunnel '\(tunnel.name)' with activationAttemptId=\(lastErrorStrings[0]): \(lastErrorStrings[1])")
...
class TunnelContainer: NSObject {
    ...
    fileprivate func startActivation(recursionCount: UInt = 0, lastError: Error? = nil, activationDelegate: TunnelsManagerActivationDelegate?) {
        ...
        do {
            ...
            let providerBundleIdentifier = (tunnelProvider.protocolConfiguration as? NETunnelProviderProtocol)?.providerBundleIdentifier ?? "<nil>"
            wg_log(.debug, message: "startActivation: Starting tunnel '\(name)' with activationAttemptId=\(activationAttemptId), providerBundleIdentifier=\(providerBundleIdentifier), isEnabled=\(tunnelProvider.isEnabled), onDemand=\(tunnelProvider.isOnDemandEnabled), status=\(tunnelProvider.connection.status)")
            try (tunnelProvider.connection as? NETunnelProviderSession)?.startTunnel(options: ["activationAttemptId": activationAttemptId])
            wg_log(.debug, message: "startActivation: startTunnel returned successfully for '\(name)' with activationAttemptId=\(activationAttemptId)")
            ...
            guard let systemError = error as? NEVPNError else {
                wg_log(.error, message: "Failed to activate tunnel '\(name)': Error: \(error) (activationAttemptId=\(activationAttemptId))")
                status = .inactive
            ...
            guard systemError.code == NEVPNError.configurationInvalid || systemError.code == NEVPNError.configurationStale else {
                wg_log(.error, message: "Failed to activate tunnel '\(name)': VPN Error: \(error), code=\(systemError.code) (activationAttemptId=\(activationAttemptId))")
                status = .inactive
