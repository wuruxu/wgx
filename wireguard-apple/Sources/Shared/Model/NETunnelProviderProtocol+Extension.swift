...
private let macOSKeychainAccessMigrationVersion = 4
private let wgQuickConfigKey = "WgQuickConfig"
...
extension NETunnelProviderProtocol {
    convenience init?(tunnelConfiguration: TunnelConfiguration, previouslyFrom old: NEVPNProtocol? = nil) {
        ...
        let wgQuickConfig = tunnelConfiguration.asWgQuickConfig()
        providerBundleIdentifier = "\(appId).network-extension"
        passwordReference = Keychain.makeReference(containing: wgQuickConfig, called: name, previouslyReferencedBy: old?.passwordReference)
        ...
        providerConfiguration = [
            "UID": getuid(),
            "KeychainAccessMigrationVersion": macOSKeychainAccessMigrationVersion,
            wgQuickConfigKey: wgQuickConfig
        ]
        #endif
    ...
    func asTunnelConfiguration(called name: String? = nil) -> TunnelConfiguration? {
        if let inlineConfig = providerConfiguration?[wgQuickConfigKey] as? String {
            return try? TunnelConfiguration(fromWgQuickConfig: inlineConfig, called: name)
        }
    ...
    func migrateConfigurationIfNeeded(called name: String) -> Bool {
         ...
         */
        if let oldConfig = providerConfiguration?[wgQuickConfigKey] as? String,
           providerConfiguration?["KeychainAccessMigrationVersion"] as? Int != macOSKeychainAccessMigrationVersion {
            #if os(macOS)
            providerConfiguration = [
                "UID": getuid(),
                "KeychainAccessMigrationVersion": macOSKeychainAccessMigrationVersion,
                wgQuickConfigKey: oldConfig
            ]
            #elseif os(iOS)
        ...
        if let passwordReference = passwordReference,
           ...
           (providerConfiguration?["KeychainAccessMigrationVersion"] as? Int != macOSKeychainAccessMigrationVersion ||
           providerConfiguration?[wgQuickConfigKey] as? String == nil),
           let config = Keychain.openReference(called: passwordReference),
           let newReference = Keychain.makeReference(containing: config, called: name, previouslyReferencedBy: passwordReference) {
            self.passwordReference = newReference
            providerConfiguration = [
                "UID": getuid(),
                "KeychainAccessMigrationVersion": macOSKeychainAccessMigrationVersion,
                wgQuickConfigKey: config
            ]
            return true
        }
        #elseif os(iOS)
