...
package com.github.wuruxu.wgx.backend;
...
import com.github.wuruxu.wgx.backend.BackendException.Reason;
import com.github.wuruxu.wgx.backend.Tunnel.State;
import com.github.wuruxu.wgx.util.SharedLibraryLoader;
...
/**
 * Implementation of {@link Backend} that uses the wgx userspace implementation to provide
 * WireGuard tunnels.
 */
@NonNullForAll
public final class WgxBackend implements Backend {
    private static final int DNS_RESOLUTION_RETRIES = 10;
    private static final String TAG = "WireGuard/WgxBackend";
    ...
    /**
     * Public constructor for WgxBackend.
     *
     * @param context An Android {@link Context}
     */
    public WgxBackend(final Context context) {
        SharedLibraryLoader.loadSharedLibrary(context, "wgx");
    ...
    @Nullable private static native String wgxGetConfig(int handle);

    private static native int wgxGetSocketV4(int handle);

    private static native int wgxGetSocketV6(int handle);

    private static native void wgxTurnOff(int handle);

    private static native int wgxTurnOn(String ifName, int tunFd, String settings);

    private static native String wgxVersion();
    ...
    public Statistics getStatistics(final Tunnel tunnel) {
        ...
        if (tunnel != currentTunnel || currentTunnelHandle == -1)
            return stats;
        final String config = wgxGetConfig(currentTunnelHandle);
        if (config == null)
            return stats;
        Key key = null;
        long rx = 0;
    ...
    /**
     * Get the version of the underlying wgx library.
     *
     * @return {@link String} value of the version of the wgx library.
     */
    @Override
    ...
    private void setStateInternal(final Tunnel tunnel, @Nullable final Config config, final State state)
        ...
        if (state == State.UP) {
            ...
            // Build config
            final String wgxConfig = config.toWgUserspaceString();
            ...
            try (final ParcelFileDescriptor tun = builder.establish()) {
                ...
                Log.d(TAG, "Wgx backend " + wgxVersion());
                currentTunnelHandle = wgxTurnOn(tunnel.getName(), tun.detachFd(), wgxConfig);
            }
            if (currentTunnelHandle < 0)
                throw new BackendException(Reason.WGX_ACTIVATION_ERROR_CODE, currentTunnelHandle);

            currentTunnel = tunnel;
            currentConfig = config;

            final int socketV4 = wgxGetSocketV4(currentTunnelHandle);
            final int socketV6 = wgxGetSocketV6(currentTunnelHandle);
            if (socketV4 >= 0)
                service.protect(socketV4);
            if (socketV6 >= 0)
                service.protect(socketV6);
        } else {
            if (currentTunnelHandle == -1) {
    ...
    /**
     * Callback for {@link WgxBackend} that is invoked when {@link VpnService} is started by the
     * system's Always-On VPN mode.
     */
    ...
    /**
     * {@link android.net.VpnService} implementation for {@link WgxBackend}
     */
    public static class VpnService extends android.net.VpnService {
        @Nullable private WgxBackend owner;
            ...
            if (owner != null) {
                ...
                if (tunnel != null) {
                    if (owner.currentTunnelHandle != -1)
                        wgxTurnOff(owner.currentTunnelHandle);
        ...
        public void setOwner(final WgxBackend owner) {
            this.owner = owner;
        }
    }
}
